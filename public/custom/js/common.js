async function uploadApi(url, formID = null, idProgressBar = null, reloadFunction = null) {
	try {
		...
		dataArr.append('arcaSecurityCsrf', Cookies.get('arcaCookieSecurityCsrf')); // csrf
...
async function callApi(url, dataObj = null, option = {}) {
	url = $('meta[name="base_url"]').attr('content') + url;
...
function noti(code = 200, text = 'Something went wrong') {
...
function noSelectDataLeft(text = 'Type', filesName = '5.png') {
	...
	var fileImage = $('meta[name="base_url"]').attr('content') + 'public/custom/img/nodata/' + filesName;
