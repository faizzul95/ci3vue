<template>
  <icon-base
    icon-name="shop-circle"
    :width="width"
    :height="height"
    :icon-color="iconColor"
  >
    <svg
      version="1.1"
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
      x="0px"
      y="0px"
      viewBox="0 0 64 64"
      style="enable-background: new 0 0 64 64"
      xml:space="preserve"
    >
      <g id="Layer_1">
        <g>
          <circle
            class="icon-shop-circle-st0"
            cx="32"
            cy="32"
            r="32"
            :style="[backgroundColor ? {} : { fill: 'none' }]"
          />
        </g>
        <g class="icon-shop-circle-st1">
          <g>
            <path
              class="icon-shop-circle-st2"
              d="M47.8,17C47.8,17,47.8,17,47.8,17c-0.1,0-0.1,0-0.1,0H32.2c-0.4,0-1.2,0.6-1.3,0.7L12.7,35.9
				c-1,1-1,2.5,0,3.5l13.9,13.9c0.5,0.5,1.1,0.7,1.7,0.7c0.6,0,1.3-0.2,1.7-0.7l18.1-18.1c0.1-0.1,0.7-0.9,0.7-1.3V18.2
				C49,17.5,48.5,17,47.8,17z M44,24c-1.1,0-2-0.9-2-2s0.9-2,2-2s2,0.9,2,2S45.1,24,44,24z"
            />
          </g>
        </g>
        <g>
          <g>
            <path
              class="icon-shop-circle-st3"
              d="M47.8,15C47.8,15,47.8,15,47.8,15c-0.1,0-0.1,0-0.1,0H32.2c-0.4,0-1.2,0.6-1.3,0.7L12.7,33.9
				c-1,1-1,2.5,0,3.5l13.9,13.9c0.5,0.5,1.1,0.7,1.7,0.7c0.6,0,1.3-0.2,1.7-0.7l18.1-18.1c0.1-0.1,0.7-0.9,0.7-1.3V16.2
				C49,15.5,48.5,15,47.8,15z M44,22c-1.1,0-2-0.9-2-2s0.9-2,2-2s2,0.9,2,2S45.1,22,44,22z"
            />
          </g>
        </g>
      </g>
      <g id="Layer_2"></g>
    </svg>
  </icon-base>
</template>

<style type="text/css">
.icon-shop-circle-st0 {
  fill: #e0e0d1;
}
.icon-shop-circle-st1 {
  opacity: 0.2;
}
.icon-shop-circle-st2 {
  fill: #231f20;
}
.icon-shop-circle-st3 {
  fill: #c75c5c;
}
</style>


<script>
import IconBase from "./IconBase.vue";

export default {
  props: {
    iconName: {
      type: String,
      default: "box",
    },
    width: {
      type: [Number, String],
      default: 24,
    },
    height: {
      type: [Number, String],
      default: 24,
    },
    iconColor: {
      type: String,
      default: "currentColor",
    },
    backgroundColor: {
      type: Boolean,
      default: true,
    },
  },
  components: { IconBase },
};
</script>
